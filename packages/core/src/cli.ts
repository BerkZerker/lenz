...
function findRoot(): string {
  ...
  const start = d;
  while (true) {
    if (existsSync(join(d, ".lenz"))) return d;
    const p = resolve(d, "..");
    if (p === d) return start;
    d = p;
  }
...
const root = findRoot();
const port = Number(
  flag("--port", process.env.LENZ_PORT ?? String(loadConfig(root).port)),
);
const api = async (
  path: string,
  body?: any,
...
const usage = `lenz — agent dev kit
  ...
  lenz index [--scip] [--rebuild]   (re)build structure.db; --scip runs scip-typescript if installed
                                    --rebuild re-extracts every file (use after changing extraction/resolution)
...
async function main() {
  switch (cmd) {
    case "init": {
      const d = initProject(root);
      console.log(`initialized ${relative(process.cwd(), d) || "."}`);
      return;
    }
    case "start": {
      const core = new Core({
        root,
        port,
        cli: [process.execPath, resolve(import.meta.path)],
      });
      core.bus.on("log", (e) => console.log(`[${e.data.level}] ${e.data.msg}`));
      core.bus.on("run.updated", (e) =>
        console.log(
          `[run] ${e.data.run.id} ${e.data.run.kind} ${e.data.run.node ?? ""} → ${e.data.run.status}`,
        ),
      );
      core.bus.on("drift.detected", (e) =>
        console.log(`[drift] ${e.data.id}: ${e.data.reasons.join("; ")}`),
      );
      core.bus.on("lock.changed", (e) =>
        console.log(
          `[lock] ${e.data.event.kind} ${e.data.event.file} ${e.data.event.run}${e.data.event.from ? " from " + e.data.event.from : ""}`,
        ),
      );
    ...
    case "index": {
      if (has("--scip")) {
        ...
        if (r.exit !== 0)
          console.error(
            `scip-typescript failed (${r.exit}): ${r.stderr.slice(-500)}`,
          );
        else console.log("index.scip written");
      }
      const rebuild = has("--rebuild");
      if (await daemonUp()) {
        const ev = await api("/index", { rebuild });
        console.log(
          `synced via daemon: +${ev.added.length} ~${ev.changed.length} -${ev.removed.length}`,
        );
        return;
      }
      const core = new Core({ root, port });
      const ev = await core.idx.indexAll(rebuild);
      const scip = core.idx.applyScip();
      console.log(
        `indexed ${core.idx.db.allFiles().length} files, ${core.idx.db.allSymbols().length} symbols, ${core.idx.db.allRefs().length} refs${scip !== null ? ` (${scip} from scip)` : ""}; +${ev.added.length} ~${ev.changed.length} -${ev.removed.length}`,
      );
      await core.close();
      return;
    }
    case "summarize": {
      await api("/summarize", { force: has("--force") });
      console.log("summarizing in the background; watch the daemon log");
      return;
    }
    case "status": {
    ...
    case "propose": {
      ...
      if (!f) throw new Error("propose <file>");
      const r = await api("/propose", {
        text: readFileSync(f, "utf8"),
        parent: flag("--parent") ?? null,
      });
      console.log(`proposed ${r.created.length} nodes`);
      return;
    }
    case "dispatch": {
      const n = await api(`/nodes/${positional[0]}/dispatch`, {
        note: flag("--note"),
      });
      console.log(`${n.id} → ${n.status} (run ${n.last_run})`);
      return;
    }
    case "verify": {
      const n = await api(`/nodes/${positional[0]}/verify`, {});
      console.log(JSON.stringify(n.verification, null, 2));
      return;
    }
    case "lock": {
      ...
      if (!file || !run)
        throw new Error("lock acquire|release <file> --run <id>");
      const rel = relative(root, resolve(file)).split("\\").join("/");
      const r = await api(
        `/locks/${sub === "release" ? "release" : "acquire"}`,
        { file: rel, run },
      );
      console.log(JSON.stringify(r));
      if (r.granted === false) process.exit(2);
      return;
    }
    case "node": {
      if (positional[0] !== "set")
        throw new Error("node set <id> <path> <value>");
      const [, id, path, ...rest] = positional;
      const value = rest.join(" ");
      const n = await api(`/nodes/${id}/set`, { path, value });
      console.log(`${n.id}.${path} set`);
      return;
    }
    case "hook": {
      await hook(positional[0]);
      return;
    }
    default:
      console.log(usage);
...
async function hook(sub: string) {
  ...
  const inRepo = files.filter(
    (f) => f && !f.startsWith("..") && !f.startsWith(".lenz/"),
  );
  const emit = (o: any) => {
        ...
        if (!r.granted) {
          emit({
            hookSpecificOutput: {
              ...
              permissionDecisionReason: `lenz lock denied: ${r.reason}`,
            },
          });
          return;
        }
...
export function bashWriteTargets(cmd: string, cwd: string): string[] {
  const writeIndicators =
    /(^|[^<>])>{1,2}\s*[^&\s]|\btee\b|\bsed\s+(-[a-zA-Z]*i|--in-place)|\bmv\b|\bcp\b|\brm\b|\bopen\([^)]*['"](w|a|r\+)|\.write\(|\bwriteFile|\btouch\b|\bpatch\b|\bgit\s+(apply|checkout|stash)\b|\bmkdir\b/;
  if (!writeIndicators.test(cmd)) return [];
  const out = new Set<string>();
  for (const m of cmd.matchAll(
    /[\w.\/-]*\.(ts|tsx|js|jsx|mjs|cjs|py|go|json|md|yaml|yml|toml|css|html)\b/g,
  )) {
    const tok = m[0];
