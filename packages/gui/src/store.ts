...
export const useStore = create<State>((set, get) => ({
  nodes: {}, tree: [], status: null, runs: [], locks: [], lockLog: [], runEvents: [], logs: [], connected: false,
  ...
  refresh: async () => {
    const [nodes, tree, status] = await Promise.all([api<LenzNode[]>("/nodes"), api<TreeItem[]>("/tree"), api<Status>("/status")]);
    set({ nodes: Object.fromEntries(nodes.map((n) => [n.id, n])), tree, status });
  },
  refreshRuns: async () => {
  ...
  connect: () => {
    ...
    ws.onmessage = (m) => {
      ...
      switch (ev.type) {
        ...
        case "structure.synced": { void api<Status>("/status").then((status) => set({ status })); break; }
