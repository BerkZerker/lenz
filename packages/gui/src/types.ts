...
export interface LenzNode {
  ...
  proposed_anchors?: (Anchor & { change: string; owner?: string | null })[]; last_run?: string; derived?: boolean; folder?: string;
}
...
export interface Status { runs: number; locks: number; drifted: number; staged: { staged: string[]; blast: string[]; immediate: boolean }; orphans: number; nodes: number; deriving: DeriveProgress | null }
