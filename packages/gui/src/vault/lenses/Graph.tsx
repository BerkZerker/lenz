...
export function GraphLens() {
  ...
  if (mode === "graph") return <GraphView onCards={() => toggle("cards")} />;
  ...
  return (
    <div className="cards-lens">
      ...
      {!children.length && <div className="hint">{focus ? "leaf node — see the detail dock below. esc to go up." : <>empty graph. press <span className="kbd">7</span> to propose, or <span className="link" onClick={deriveGraph}>generate the graph from code</span> (<span className="kbd">g</span>).</>}</div>}
